...
export class BusinessSignupChange {
    ...
    this.http.get('http://localhost:3001/api/user/biz/'+this.decodedJwt.idx, {headers:contentHeaders}) //서버로부터 필요한 값 받아오기
      ...
      .subscribe(
        response => {
          ...
          this.contact = this.data.user.contact;
          this.companyName = this.data.user.companyName;
          this.ownerName = this.data.user.ownerName;
          this.bizRegNo = this.data.user.bizRegNo;
          this.workPlace = this.data.user.workPlace;
          this.mainWorkField = this.data.user.mainWorkField;
          this.mailWorkArea = this.data.user.mailWorkArea;
        },
        error => {
          alert(error.text());
  ...
  businesssignupchange(event, email, password, password_ok, contact, companyName, ownerName, bizRegNo, workPlace, mainWorkField, mailWorkArea, memberType) {
    //html에서의 value값
    ...
    if (passwords != confirmpasswords) {
      ...
      let body = JSON.stringify({ email, password, contact, companyName, ownerName, bizRegNo, workPlace, mainWorkField, mailWorkArea, memberType });
